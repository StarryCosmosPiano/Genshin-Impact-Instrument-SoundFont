...
**名称说明：**原文件中的“风物琴”对应 **Windsong Lyre（风物之诗琴）**；“谐律钢琴”对应 **Harmonic Keys（谐律键琴）**。**Ukulele** 的游戏中文名称为“悠可琴”，乐器通用译名为“尤克里里”。**Vodyanitsa（沃雅妮莎）** 为人声音源。[查看完整改名对照及名称参考](docs/File-Names.zh-CN.md)。
...
**预设选择：**所有音源的 Bank 均为 **0**。`Ukulele (Original with Chords).sf2` 的 Program 为 **1**，其他文件的 Program 均为 **0**。这里从 0 开始编号；如果软件从 1 开始显示，界面中可能分别显示为 **2** 和 **1**。
...
13 个音源的采样数据、预设和内嵌信息均与原始文件一致。[SHA256SUMS.txt](SHA256SUMS.txt) 提供校验值，[catalog.json](catalog.json) 记录音源名称、版本、大小和 MIDI 映射信息。

合集由 [StarryCosmosPiano](https://github.com/StarryCosmosPiano) 维护。原始游戏音频来自 **《原神》／HoYoverse／miHoYo**。所提供音源的内嵌作者字段包含 **StarryCosmos** 与 **Best**，文件内署名均已保留。
